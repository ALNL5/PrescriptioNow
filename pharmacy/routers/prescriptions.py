...
@router.get("/prescriptions", response_model=Union[List[PrescriptionOut], Error])
def get_all_customers_prescriptions(repo: PrescriptionRepository = Depends()):
    return repo.get_all()

@router.get("/prescriptions/orders", response_model=Union[List[PrescriptionOut], Error])
def get_ordered_prescriptions(repo: PrescriptionRepository = Depends()):
    return repo.get_ordered()

@router.put("/prescriptions/{prescription_id}", response_model=Union[PrescriptionOut, Error])
def update_prescription(prescription_id: int, prescription: PrescriptionIn, repo: PrescriptionRepository=Depends()) -> Union[PrescriptionOut, Error]:
    return repo.update(prescription_id, prescription)


@router.delete("/prescriptions/{prescription_id}", response_model=bool)
def delete_prescription(prescription_id: int, repo: PrescriptionRepository=Depends()) -> bool:
    return repo.delete(prescription_id)


@router.get("/prescriptions/{prescription_id}", response_model=Union[PrescriptionOut, Error])
def get_one_prescription(prescription_id: int, response: Response, repo: PrescriptionRepository=Depends()) -> PrescriptionOut:
...
def delete_employee(employee_id: int, repo: EmployeeRepository=Depends()) -> bool:
    return repo.delete_employee(employee_id)

@router.put("/employees/{employee_id}", response_model=Union[EmployeesOut, Error])
def update_employee(employee_id: int, employee: EmployeesIn, repo: EmployeeRepository=Depends()) -> Union[EmployeesOut, Error]:
